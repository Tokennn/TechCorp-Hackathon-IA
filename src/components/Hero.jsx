...
export default function Hero({ onSend }) {
  return (
    <section className="relative isolate overflow-hidden">
      ...
      <div className="mx-auto flex min-h-[88vh] w-full max-w-3xl flex-col items-center px-5 pt-16 pb-24 sm:pt-24">
        <AnimatedContent distance={40} duration={0.8} ease="power3.out" delay={0}>
          <div className="mb-7 inline-flex items-center gap-2 rounded-full border border-brand-300/60 bg-surface/80 py-1.5 pr-4 pl-1.5 text-sm shadow-sm backdrop-blur dark:border-white/10 dark:bg-white/5">
        ...
        <AnimatedContent distance={50} duration={0.9} ease="power3.out" delay={0.1}>
          <h1 className="text-center font-display text-4xl font-extrabold leading-[1.05] tracking-tight text-brand-900 sm:text-5xl md:text-6xl dark:text-brand-50">
            ...
            <span className="bg-gradient-to-r from-brand-700 to-brand-400 bg-clip-text text-transparent">
              l'IA financière TechCorp
            </span>
          </h1>
        </AnimatedContent>

        <AnimatedContent distance={40} duration={0.8} ease="power3.out" delay={0.2}>
          <p className="mt-5 max-w-xl text-center text-base text-brand-800/70 sm:text-lg dark:text-brand-200/80">
